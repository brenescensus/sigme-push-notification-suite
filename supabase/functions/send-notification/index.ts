...
//       new URL(subscriber.endpoint).origin,
//       vapidPublicKey,
//       vapidPrivateKey,
//       websiteUrl
//     );

//     // Use Web Push Protocol "aesgcm" (not aes128gcm)
...
//   // const sub = 'mailto:push@sigme.app';
...
/**
 * Send Notification Edge Function – FIXED & COMPLIANT
 *
 * - Web Push (RFC 8291 / 8292) – browser-safe
 * - Android FCM HTTP v1
 * - No broken crypto
 * - No silent drops
 * 
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const vapidPublicKey = Deno.env.get("FIREBASE_VAPID_PUBLIC_KEY")!;
    const vapidPrivateKey = Deno.env.get("FIREBASE_VAPID_PRIVATE_KEY")!;

    if (!supabaseUrl || !serviceKey || !anonKey || !vapidPublicKey || !vapidPrivateKey) {
    ...
    if (!website || website.user_id !== user.id) {
      return json({ error: "Unauthorized website" }, 403);
    }
    ...
    const webSubs = subscribers.filter(s => s.endpoint && s.p256dh_key && s.auth_key);
    const androidSubs = subscribers.filter(s => s.fcm_token);
    ...
    for (const sub of androidSubs) {
      const result = await sendFCM(sub.fcm_token, notification);
      if (result.success) sent++;
      else failed++;
    }
...
/* ───────────────────────────────────────────── */
/* WEB PUSH – CORRECT IMPLEMENTATION              */
/* ───────────────────────────────────────────── */
...
async function sendWebPush(
  ...
  const audience = new URL(subscriber.endpoint).origin;

  const vapidJwt = await createVapidJwt(audience, vapidPublicKey, vapidPrivateKey, siteUrl);
  ...
  const res = await fetch(subscriber.endpoint, {
    method: "POST",
    headers: {
      "Authorization": `vapid t=${vapidJwt}, k=${vapidPublicKey}`,
      "Content-Encoding": "aesgcm",
  ...
  if ([404, 410].includes(res.status)) return { success: false, error: "SUBSCRIPTION_EXPIRED" };

  return { success: false, error: await res.text() };
}

/* ───────────────────────────────────────────── */
/* CRYPTO – CLEAN & RFC COMPLIANT                 */
/* ───────────────────────────────────────────── */

async function encryptWebPushPayload(payload: string, p256dh: string, auth: string) {
  const userKey = base64UrlDecode(p256dh);
  ...
  const ikm = await hkdf(authKey, new Uint8Array(sharedSecret), "Content-Encoding: auth\0", 32);
  const cek = await hkdf(salt, ikm, "Content-Encoding: aesgcm\0", 16);
...
async function hkdf(salt: Uint8Array, ikm: Uint8Array, info: string, length: number) {
  ...
  const input = new Uint8Array([...infoBuf, 1]);
  const okm = await crypto.subtle.sign("HMAC", await crypto.subtle.importKey("raw", prk, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]), input);
  return new Uint8Array(okm).slice(0, length);
}
